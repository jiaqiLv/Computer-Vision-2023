function resultImage = mergeTwoImages(imgColor1, imgColor2)
    ...
    for row = 1:mergeRows
        for col = 1: mergeCols
            ...
            if transformedImage(row,col,1) ~=0
                oldPixel = 1;
                oldR = transformedImage(row,col,1);
                oldG = transformedImage(row,col,2);
                oldB = transformedImage(row,col,3);
            end
            ...
            if (floorX >= 1 && floorY >=1 && ceilX <= colsIm1 && ceilY <= rowsIm1) 
                ...
                if oldPixel
                    transformedImage(row,col,1) = (transformedImage(row,col,1) + oldR) /2;
                    transformedImage(row,col,2) = (transformedImage(row,col,2) + oldG) /2;
                    transformedImage(row,col,3) = (transformedImage(row,col,3) + oldB) /2;
                end
            end
        end
    end
    ...
    resultImage = uint8(transformedImage);
